...
import { ArrowLeft, User, Users, Bell, Eye, Search, Plus } from "lucide-react"
...
function TasksPageContent() {
  ...
  const [selectedSortCategory, setSelectedSortCategory] = useState<'status' | 'tag' | 'creator' | 'assignee' | null>(null)
  ...
  const weekGroups = useMemo(() => {
    const groups = []
    let currentGroup: typeof allDays = []
  ...
  }, [currentWeekOffset, allDays, today, loadedWeeksRange])
  ...
  return (
    <div className="h-screen bg-white flex flex-col">
          ...
          {!isSearchActive ? (
            <>
              ...
              <div className="flex items-center gap-1">
                ...
                {/* View Toggle (Eye for Tasks) */}
                <button 
                  className={headerBtnClass}
                  onClick={() => setIsViewTasksSheetVisible(true)}
                >
                  <Eye className="h-[30px] w-[30px] text-red-600" strokeWidth={2} />
                </button>
      ...
      {/* 2. MINI CALENDAR STRIP - Three weeks for smooth dragging */}
      <div 
        ref={miniCalendarRef}
      ...
      <ViewTasksSheet
        ...
        onSelectSortCategory={(category) => setSelectedSortCategory(category as any)}
